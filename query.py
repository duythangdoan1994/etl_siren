...
def process_json():
    ...
    for i in collection:
        if i == 'feedbacks' or i == 'call_logs':
            if i == 'feedbacks':
                with open("./data/%s.json" % i, "wb") as f:
            ...
            #             u = "{:%d/%m/%Y/%H/%M/%S}".format(document['updated_at'])
            #             c = "{:%d/%m/%Y/%H/%M/%S}".format(document['updated_at'])
            #             document['updated_at'] = time.mktime(datetime.datetime.strptime(u, "%d/%m/%Y/%H/%M/%S").timetuple())
            #             document['created_at'] = time.mktime(datetime.datetime.strptime(c, "%d/%m/%Y/%H/%M/%S").timetuple())
            #             document = json_util.dumps(document)
            #             document = json.dumps(document, default=json_util.default)
            #             json.dump(f, document)
...
#                 os.system('split -b60m %s ./data/feedb' % f)
#                 os.system('rm -rf %s' % f)
